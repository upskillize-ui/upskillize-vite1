 ...
 *   autoOpen     – Delay (ms) before the bubble auto-opens. Set to 0 to disable. Default 4000.
 */
export default function WhatsAppChat({
  ...
  // Auto-open bubble after delay
  useEffect(() => {
    if (!autoOpen) return;
    const timer = setTimeout(() => setOpen(true), autoOpen);
  ...
  return (
    <>
      ...
      {/* Floating button + bubble — fixed to bottom-right */}
      <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
        ...
        {/* ── FAB Button ── */}
        <div className="relative">
          {/* Pulse rings (only while pulsing) */}
          {pulse && (
            <>
          ...
          <button
            ...
            className="wa-float-btn relative w-12 h-12 rounded-full bg-[#25D366] hover:bg-[#1ebe5d] shadow-lg hover:shadow-xl flex items-center justify-center transition-colors"
          >
            {open ? (
              /* Close icon */
